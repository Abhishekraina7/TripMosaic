...
''';
      ...
      print(newItineraryData);

      // Ensure the response is a Map, decode if it's a String
      dynamic parsedData = newItineraryData;
      if (parsedData is String) {
        try {
          parsedData = jsonDecode(parsedData);
        } catch (e) {
          print('Error decoding JSON string: $e');
          throw Exception('Failed to decode Gemini API response as JSON.');
        }
      }
      if (parsedData is! Map<String, dynamic>) {
        throw Exception('Gemini API response is not a valid JSON object.');
      }

      final itinerary = _convertJsonToTrip(parsedData);
